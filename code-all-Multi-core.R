...
scadThreshold <- function(entry, lambda, a) {
  ...
  entry[e2] <- ((a - 1) * entry[e2] - sign(entry[e2]) * a * lambda) / (a - 1)
...
w_init <- rep(1/sqrt(ncol(dataset)), ncol(dataset))
sample_run_1 <- RSKOD(k = 3, c = 200, lambda = 1.5, dataset = dataset, w = w_init)





c_var <- sort(2^seq(log2(100), log2(200), length = 5))
lam_var <- c(1.1:5.1)
